...
const sqlPath = path.join(root, 'db/migrations/002_rca_and_timestamps.sql');
...
const [cols] = await conn.query(`
  ...
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'activities'
    AND COLUMN_NAME IN ('opened_at','closed_at','priority')
  ORDER BY COLUMN_NAME
`);
const [tables] = await conn.query(`
  SELECT TABLE_NAME FROM information_schema.TABLES
  WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'root_cause_analysis'
`);
const [rca] = await conn.query(`SELECT COUNT(*) AS n FROM root_cause_analysis`);
console.log({ activityCols: cols, rcaTable: tables, rcaRows: rca });
await conn.end();
console.log(`Applied ${statements.length} statements from 002.`);
